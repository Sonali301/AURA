...
export default function AutonomousDecisionTrace({ incident }) {
  ...
  const decisionPath = incident.decision_path || 'AUTO_ESCALATED';
  ...
  const generateTimeline = () => {
    ...
    return [
      ...
      { time: timeStr, event: decisionPath === 'AUTO_HEAL' ? "Autonomous execution approved" : decisionPath === 'HUMAN_APPROVAL' ? "Execution requires human oversight" : "Execution automatically escalated" }
    ];
  };
                ...
                <div className="bg-[#050510] border border-neon-cyan/30 p-4 rounded font-mono relative overflow-hidden group shadow-[0_0_15px_rgba(0,240,255,0.05)]">
                  ...
                  <div className="pr-24">
                    ...
                    <div className="flex space-x-6 border-t border-neon-cyan/20 pt-3">
                      ...
                      <div>
                        <span className="text-[9px] text-gray-500 uppercase">Risk Assessment:</span>
                        <span className={`text-xs ml-1.5 font-bold ${
                          incident.risk_level === 'CRITICAL' || incident.risk_level === 'HIGH' ? 'text-neon-pink drop-shadow-[0_0_8px_rgba(255,0,85,0.5)]' :
                          incident.risk_level === 'MEDIUM' ? 'text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]' :
                          'text-neon-green drop-shadow-[0_0_8px_rgba(0,255,102,0.5)]'
                        }`}>{incident.risk_level || 'UNKNOWN'}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mb-6">
                ...
                {decisionPath === 'AUTO_HEAL' ? (
                  <div className="bg-neon-green/10 border border-neon-green/30 p-3.5 rounded flex items-start space-x-3 shadow-[0_0_15px_rgba(0,255,102,0.1)]">
                    ...
                    <div>
                      <div className="text-xs font-bold text-neon-green uppercase tracking-wide mb-1 drop-shadow-[0_0_8px_rgba(0,255,102,0.8)]">✔ Action Auto-Approved</div>
                      <div className="text-xs text-gray-300 font-mono leading-relaxed">
                        {incident.decision_reason || incident.validation_reason || "Confidence threshold satisfied. Action present in approved recovery allowlist."}
                      </div>
                    </div>
                  </div>
                ) : decisionPath === 'HUMAN_APPROVAL' ? (
                  <div className="bg-yellow-400/10 border border-yellow-400/30 p-3.5 rounded flex items-start space-x-3 shadow-[0_0_15px_rgba(250,204,21,0.1)]">
                    <ShieldAlert size={18} className="text-yellow-400 mt-0.5 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]" />
                    <div>
                      <div className="text-xs font-bold text-yellow-400 uppercase tracking-wide mb-1 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]">⚠ Human Approval Required</div>
                      <div className="text-xs text-gray-300 font-mono leading-relaxed">
                        {incident.decision_reason || incident.validation_reason || "Action deemed sensitive by safety guardrails. Escalated for human review."}
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="bg-neon-purple/10 border border-neon-purple/30 p-3.5 rounded flex items-start space-x-3 shadow-[0_0_15px_rgba(176,38,255,0.1)]">
                    <ShieldAlert size={18} className="text-neon-purple mt-0.5 drop-shadow-[0_0_8px_rgba(176,38,255,0.8)]" />
                    <div>
                      <div className="text-xs font-bold text-neon-purple uppercase tracking-wide mb-1 drop-shadow-[0_0_8px_rgba(176,38,255,0.8)]">✖ Action Auto-Escalated</div>
                      <div className="text-xs text-gray-300 font-mono leading-relaxed">
                        {incident.decision_reason || incident.validation_reason || "Confidence below threshold or cooldown exceeded. Auto-escalated."}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="bg-black/40 border border-white/5 p-3.5 rounded space-y-2">
                  {thoughtTimeline.map((ev, i) => (
                    ...
                    >
                      ...
                      <span className={`text-gray-400 ${i === thoughtTimeline.length - 1 ? (decisionPath === 'AUTO_HEAL' ? 'text-neon-green font-bold drop-shadow-[0_0_5px_rgba(0,255,102,0.8)]' : decisionPath === 'HUMAN_APPROVAL' ? 'text-yellow-400 font-bold drop-shadow-[0_0_5px_rgba(250,204,21,0.8)]' : 'text-neon-purple font-bold drop-shadow-[0_0_5px_rgba(176,38,255,0.8)]') : ''}`}>
                        {ev.event}
                      </span>
                    </motion.div>
                  ))}
